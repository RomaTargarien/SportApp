...
class EmailChangeFragment : Fragment() {
    ...
    private lateinit var viewModel: EmailChangeViewModel
    ...
    override fun onCreateView(
        ...
        viewModel = ViewModelProvider(requireActivity()).get(EmailChangeViewModel::class.java)
    ...
    override fun onResume() {
        ...
        disposes.add(viewModel.emailChangingState.observeOn(AndroidSchedulers.mainThread()).subscribe({
            when (it) {
                is Resource.Success -> {
                    viewModel.goToLoginScreen.onNext(Unit)
                }
                is Resource.Error -> {
